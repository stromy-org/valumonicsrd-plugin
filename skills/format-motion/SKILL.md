---
name: format-motion
description: "Author tasteful, brand-aware motion across Stromy''s HTML surfaces — a substrate-agnostic library of motion primitives (staggered reveal, line-draw, count-up, motion-path, scroll-reveal, element morph) plus the restraint doctrine that keeps motion meaningful. Two modes: embed-live (inline a primitive into a deck or site you are hand-authoring) and bake (author a self-contained HTML animation and turn it into MP4/GIF/PNG frames via the render_motion MCP tool, for PowerPoint or PDF). USE THIS whenever someone wants animation, an animated reveal or transition, a line that draws itself, a counting KPI, an animated explainer, a baked clip from web animation, or a deck or site that feels alive without becoming gimmicky. Brand-optional: reads the brand''s motion grammar from companies/<slug>/brand_context.json when present. Siblings: numeric data-viz → format-chart; structural diagrams → format-diagram; full branded explainer video → format-video-hd."
